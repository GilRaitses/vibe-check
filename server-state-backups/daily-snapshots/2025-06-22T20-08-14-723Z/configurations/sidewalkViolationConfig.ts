...
export function parseSidewalkViolationResponse(response: string): SidewalkViolationMatrix {
  ...
  // Ensure proper number typing by explicit casting
  return {
    cyclist_speed: numbers[0] as number,
    cyclist_direction: numbers[1] as number,
    cyclist_awareness: numbers[2] as number,
    cyclist_equipment: numbers[3] as number,
    pedestrian_density: numbers[4] as number,
    pedestrian_reaction: numbers[5] as number,
    pedestrian_vulnerable: numbers[6] as number,
    sidewalk_width: numbers[7] as number,
    bike_lane_proximity: numbers[8] as number,
    pedestrian_infrastructure: numbers[9] as number,
    safety_risk: numbers[10] as number,
    violation_clarity: numbers[11] as number,
    repeat_location: numbers[12] as number
  };
}
