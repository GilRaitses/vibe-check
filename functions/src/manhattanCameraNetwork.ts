...
import { CameraMetadata, VoronoiTerritory, ManhattanCameraNetwork } from '../../shared/types';
 ...
 */
function generateManhattanStreetGrid(): Array<{lat: number, lng: number, weight: number, name: string}> {
  const intersections: Array<{lat: number, lng: number, weight: number, name: string}> = [];
  ...
  const streets: Array<{lat: number, name: string}> = [];
 ...
 */
function isValidManhattanLocation(lat: number, lng: number): boolean {
  // Basic bounds check
 ...
 */
function generateCameraLocations(targetCount: number): Array<{lat: number, lng: number, name: string, type: string}> {
  const cameras = [];
 ...
 */
function createCameraMetadata(id: string, location: {lat: number, lng: number, name: string, type: string}): CameraMetadata {
  const neighborhoods = [
 ...
 */
function generateVoronoiTerritory(camera: CameraMetadata, allCameras: CameraMetadata[]): VoronoiTerritory {
 ...
 */
export function generateManhattanCameraNetwork(cameraCount: number = 300): ManhattanCameraNetwork {
  ...
  const cameras: { [cameraId: string]: CameraMetadata } = {};
  ...
  const territories: { [territoryId: string]: VoronoiTerritory } = {};
  ...
  const network: ManhattanCameraNetwork = {
    territories,
    cameras,
    network_metadata: {
...
function generateCoverageStreets(location: {lat: number, lng: number, name: string}): string[] {
...
function findNearestNeighbors(camera: CameraMetadata, allCameras: CameraMetadata[], count: number): CameraMetadata[] {
  return allCameras
    .filter(c => c.id !== camera.id)
    .map(c => ({
      ...c,
      distance: calculateDistance(camera.location, c.location)
    }))
...
function generateVoronoiPolygon(camera: CameraMetadata, neighbors: CameraMetadata[]): {coordinates: number[][]} {
...
function calculatePolygonPerimeter(coordinates: number[][]): number {
  let perimeter = 0;
  for (let i = 0; i < coordinates.length - 1; i++) {
...
function calculateBounds(coordinates: number[][]): {north: number, south: number, east: number, west: number} {
...
function calculateSharedBorderLength(camera1: CameraMetadata, camera2: CameraMetadata): number {
  return calculateDistance(camera1.location, camera2.location) * 0.3; // Approximate shared border
}

function calculateDirection(camera1: CameraMetadata, camera2: CameraMetadata): 'north' | 'south' | 'east' | 'west' | 'northeast' | 'northwest' | 'southeast' | 'southwest' {
...
function isBorderTerritory(camera: CameraMetadata): boolean {
...
function isWaterAdjacent(camera: CameraMetadata): boolean {
