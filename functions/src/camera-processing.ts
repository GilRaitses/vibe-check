 ...
 */
export async function getCameraUuid(cameraData: any, db?: admin.firestore.Firestore): Promise<string | null> {
  try {
    ...
    try {
      ...
      if (fs.existsSync(zoneLookupPath)) {
        ...
        const cameraId = cameraData.camera_id || cameraData.camera?.id;
        const cameraHandle = cameraData.camera?.handle;
 ...
 */
export async function getCameraZoneInfo(cameraData: any): Promise<any> {
  try {
    const fs = require('fs');
    const path = require('path');
    ...
    return {
      ...
      handle: cameraData.camera?.handle || 'Unknown',
      camera_name: cameraData.camera?.name || 'Unknown Camera',
      borough: cameraData.camera?.area || 'Unknown',
      coordinates: [cameraData.camera?.longitude || 0, cameraData.camera?.latitude || 0],
      latitude: cameraData.camera?.latitude || 0,
      longitude: cameraData.camera?.longitude || 0
    };
 ...
 */
export async function fetchNYCCameraImage(uuid: string): Promise<Buffer> {
  try {
...
export async function processImageWithVision(imageBuffer: Buffer): Promise<any> {
  try {
    ...
    try {
      ...
      if (visionError.message?.includes('API has not been used') || 
          visionError.message?.includes('not enabled') ||
      ...
      if (visionError.message?.includes('authentication') || 
    ...
    const objectCounts = {
      ...
      vehicle_count: objects.filter(obj => 
        ...
        obj.name?.toLowerCase().includes('truck') ||
    ...
    const infrastructure = {
      sidewalk_visible: labels.some(label => label.description?.toLowerCase().includes('sidewalk')),
      bike_lane_visible: labels.some(label => label.description?.toLowerCase().includes('bike')),
      crosswalk_visible: labels.some(label => label.description?.toLowerCase().includes('crosswalk')),
      traffic_signals_visible: labels.some(label => 
        label.description?.toLowerCase().includes('traffic') && 
        label.description?.toLowerCase().includes('light')
      ),
      street_lighting: labels.some(label => label.description?.toLowerCase().includes('street light'))
    };
    ...
    const result = {
      ...
      cloud_vision_data: {
        ...
        infrastructure,
        detected_objects: objects.map(obj => ({
          name: obj.name,
 ...
 * Calculate safety score based on detected elements
 */
function calculateSafetyScore(objectCounts: any, infrastructure: any, labels: any[]): number {
  let score = 5; // Base safety score
  
  // Reduce score for high pedestrian/vehicle density
  if (objectCounts.pedestrian_count > 5) score -= 1;
  if (objectCounts.vehicle_count > 8) score -= 1;
  
  // Improve score for good infrastructure
  if (infrastructure.sidewalk_visible) score += 0.5;
  if (infrastructure.traffic_signals_visible) score += 0.5;
  if (infrastructure.crosswalk_visible) score += 0.5;
  
  // Check for safety-related labels
  const dangerLabels = labels.filter(label => 
    label.description?.toLowerCase().includes('construction') ||
    label.description?.toLowerCase().includes('crowd') ||
    label.description?.toLowerCase().includes('congestion')
  );
