...
const geminiAI = new GoogleGenerativeAI(functions.config().gemini?.api_key || 'demo-key');
 ...
 */
app.get('/monitoring/processing-tracker', async (req: Request, res: Response) => {
  try {
    // Get current processing queue
    const queueSnapshot = await db.collection('processing_queue')
      .orderBy('priority', 'desc')
      .orderBy('scheduled_time', 'asc')
      .limit(10)
      .get();
    
    const upcomingZones = queueSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
      status: 'queued'
    }));
    ...
    const recentSnapshot = await db.collection('processing_tracker')
    ...
    const processingSnapshot = await db.collection('processing_tracker')
      .where('status', '==', 'processing')
      .get();
    
    const currentlyProcessing = processingSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
 ...
 */
app.post('/monitoring/trigger-analysis/:cameraId', async (req: Request, res: Response) => {
  const { cameraId } = req.params;
...
async function updateProcessingTracker(cameraId: string, data: any) {
  try {
    await db.collection('processing_tracker').doc(cameraId).set({
      ...data,
      camera_id: cameraId,
      last_updated: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
...
async function getProcessingStats() {
  try {
    const totalCameras = (await db.collection('monitoring_schedules').get()).size;
    const totalAnalyses = (await db.collection('analyses').get()).size;
...
async function processImageAnalysisAsync(cameraId: string) {
  try {
...
app.post('/gemini/system-analysis', async (req: Request, res: Response) => {
  try {
...
async function getRealSystemContext() {
  try {
    const context: {
      timestamp: string;
      database_collections: Record<string, any>;
    ...
    for (const collectionName of collections) {
      try {
        ...
        context.database_collections[collectionName] = {
          count: snapshot.size,
          last_updated: docs.length > 0 ? (docs[0] as any).created_at || (docs[0] as any).timestamp : null,
    ...
    context.summary = {
      total_cameras: context.database_collections['monitoring_schedules']?.count || 0,
      total_zones: context.database_collections['voronoi_territories']?.count || 0,
      total_analyses: context.database_collections['analyses']?.count || 0,
...
app.get('/database/firebase-viewer', async (req: Request, res: Response) => {
  try {
    const collections = await Promise.all([
      db.collection('monitoring_schedules').limit(5).get(),
      db.collection('voronoi_territories').limit(5).get(), 
      db.collection('analyses').limit(5).get(),
      db.collection('violation_events').limit(5).get()
    ]);

    const databaseView = {
      monitoring_schedules: collections[0].docs.map(doc => ({ id: doc.id, ...doc.data() })),
      voronoi_territories: collections[1].docs.map(doc => ({ id: doc.id, ...doc.data() })),
      analyses: collections[2].docs.map(doc => ({ id: doc.id, ...doc.data() })),
      violation_events: collections[3].docs.map(doc => ({ id: doc.id, ...doc.data() }))
    };
...
app.get('/database/bigquery-viewer', async (req: Request, res: Response) => {
  try {
...
app.get('/system/architecture-viewer', async (req: Request, res: Response) => {
  try {
