...
import { AdaptiveMonitoringEngine } from './adaptiveMonitoringEngine';
import { generateManhattanCameraNetwork } from './manhattanCameraNetwork';
 ...
 */
app.post('/gemini-agent-test', async (req: Request, res: Response) => {
  try {
 ...
 */
app.get('/test-collection/:collection', async (req: Request, res: Response) => {
  try {
 ...
 * Store Metrics
 */
app.post('/store-metrics', async (req: Request, res: Response) => {
  try {
    const metrics = req.body;
    await db.collection('statistical_metrics').add({
      ...metrics,
      stored_timestamp: admin.firestore.FieldValue.serverTimestamp()
    });
 ...
 */
app.get('/retention-status', (req: Request, res: Response) => {
  return res.json({
    retention_policies: {
 ...
 */
app.get('/query-performance-test', async (req: Request, res: Response) => {
  try {
 ...
 */
app.get('/metrics/charts/violation-rates', (req: Request, res: Response) => {
  return res.json({
    chart_data: {
...
app.get('/metrics/charts/trend-analysis', (req: Request, res: Response) => {
  return res.json({
    chart_data: {
...
app.get('/metrics/charts/location-comparison', (req: Request, res: Response) => {
  return res.json({
    chart_data: {
...
app.get('/metrics/charts/time-series', (req: Request, res: Response) => {
  return res.json({
    chart_data: {
 ...
 * Analysis by ID
 */
app.get('/analysis/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 * Recent Analysis
 */
app.get('/metrics/recent-analysis', async (req: Request, res: Response) => {
  try {
    const snapshot = await db.collection('analyses')
      .orderBy('timestamp', 'desc')
      .limit(10)
 ...
 * Recent Alerts
 */
app.get('/alerts/recent', async (req: Request, res: Response) => {
  try {
 ...
 * Calculate Trends
 */
app.get('/metrics/calculate-trends', async (req: Request, res: Response) => {
  try {
 ...
 * Location Metrics
 */
app.get('/metrics/location/:location', async (req: Request, res: Response) => {
  try {
    const { location } = req.params;
    ...
    return res.json({
      ...
      avg_safety_score: analyses.length > 0 
        ? analyses.reduce((sum, a: any) => sum + (a.analysis?.safetyScore || 5), 0) / analyses.length 
        : 5,
 ...
 */
app.post('/ml-predict', async (req: Request, res: Response) => {
  try {
    ...
    const bikeRedLight = req.body.numerical_data?.[2] || 0;
    return res.json({
      success: true,
      decision: {
...
async function storeViolationData(numerical_data: number[], location: string): Promise<void> {
  try {
...
async function predictViolationSeverity(numerical_data: number[]): Promise<any> {
  try {
    const query = `
      SELECT 
    ...
    if (rows.length > 0) {
      const row = rows[0];
      const probabilities = row.predicted_violation_severity_probs as Record<string, number>;
...
async function forecastViolations(location: string): Promise<any> {
  try {
    const query = `
      SELECT 
        forecast_timestamp,
...
async function getModelStats(): Promise<any> {
  try {
    const query = `
      SELECT 
 ...
 */
app.post('/monitoring/initialize', async (req: Request, res: Response) => {
  try {
...
app.get('/monitoring/status', async (req: Request, res: Response) => {
  try {
    ...
    const next_analyses = schedules
      .filter((schedule: any) => schedule.next_analysis_time)
      .sort((a: any, b: any) => {
        const timeA = a.next_analysis_time?.seconds || 0;
        const timeB = b.next_analysis_time?.seconds || 0;
        return timeA - timeB;
      })
      .slice(0, 10)
      .map((schedule: any) => ({
        camera_id: schedule.camera_id as any as any,
        camera_name: schedule.camera?.name || 'Unknown',
 ...
 */
app.post('/monitoring/process', async (req: Request, res: Response) => {
  try {
    ...
    const schedulesToProcess = dueSchedulesSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
    ...
    for (const schedule of schedulesToProcess) {
      try {
        ...
        await db.collection('monitoring_schedules').doc(schedule.id).update({
          ...
          current_tier: analysisResult.updated_tier || (schedule as any).current_tier,
 ...
 */
app.post('/monitoring/analyze/:camera_id', async (req: Request, res: Response) => {
  try {
 ...
 */
app.get('/monitoring/schedule/:camera_id', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    ...
    return res.json({
      ...
      statistics: {
        ...
        avg_severity: recent_violations.length > 0 
          ? recent_violations.reduce((sum: number, v: any) => sum + (v.severity_score || 0), 0) / recent_violations.length 
          : 0,
        last_violation: recent_violations.length > 0 ? (recent_violations[0] as any).timestamp || null : null
      }
    });
 ...
 */
app.put('/monitoring/tier/:camera_id', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    ...
    const old_tier = schedule?.current_tier;
    ...
    const next_analysis_time = admin.firestore.Timestamp.fromDate(
 ...
 */
app.get('/monitoring/timeseries/:camera_id', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    const { hours = 24 } = req.query;
    
    const hoursBack = Math.min(parseInt(hours as string) || 24, 168); // Max 1 week
    ...
    violations.forEach((violation: any) => {
      const timestamp = violation.timestamp?.toDate ? violation.timestamp.toDate() : new Date(violation.timestamp);
      ...
      if (violation.violation_types) {
        violation.violation_types.forEach((type: string) => {
    ...
    const timeSeriesData = Object.values(hourlyData).map((hour: any) => ({
      ...hour,
      avg_severity: hour.violation_count > 0 ? hour.total_severity / hour.violation_count : 0,
      violation_types: Array.from(hour.violation_types)
    }));
    ...
    return res.json({
      ...
      summary: {
        ...
        peak_hour: timeSeriesData.reduce((max, hour) => 
          hour.violation_count > max.violation_count ? hour : max, 
          { violation_count: 0, timestamp: null }
        ),
        avg_severity_overall: violations.length > 0 
          ? violations.reduce((sum: number, v: any) => sum + (v.severity_score || 0), 0) / violations.length 
          : 0
      }
    });
...
async function identifyCriticalCameraZones(): Promise<any[]> {
...
async function processScheduledCameraAnalysis(schedule: any): Promise<any> {
  console.log(`🔍 Processing analysis for camera ${schedule.camera?.name || schedule.camera_id as any as any}`);
  
  try {
    // Use GOOGLE CLOUD VISION API for real analysis
    console.log(`🔥 [CLOUD_VISION] Calling Google Cloud Vision API for camera ${schedule.camera_id as any as any}`);
    ...
    const cloudVisionAnalysis = await CloudVisionService.analyzeCameraFeed(camera.imageUrl);
    ...
    await storeViolationData(numericalData, camera.name || schedule.camera_id as any as any);
    ...
    console.error(`❌ Cloud Vision analysis failed for ${schedule.camera_id as any as any}:`, error);
...
function extractViolationTypesFromCloudVision(analysis: any): string[] {
  const types: string[] = [];
  
  // Extract from Cloud Vision analysis
  if (analysis.pedestrian_count >= 3) types.push('pedestrian_congestion');
  if (analysis.bicycle_count >= 2) types.push('bicycle_presence');
  if (analysis.vehicle_count >= 3) types.push('vehicle_congestion');
  if (analysis.pedestrian_bike_interaction >= 2) types.push('pedestrian_bike_interaction');
  if (analysis.pedestrian_bike_interaction >= 3) types.push('dangerous_bike_lane_position');
  if (analysis.pedestrian_bike_interaction >= 4) types.push('critical_bike_red_light_violation');
  ...
  if (analysis.overall_safety_score <= 3) types.push('critical_safety_risk');
  if (analysis.overall_safety_score <= 5) types.push('high_risk_detected');
  
  return types;
}

function getTimeOfDayCategory(date: Date): string {
    ...
    try {
      ...
      for (const schedule of schedulesToProcess) {
        try {
          ...
          console.log(`✅ Processed analysis for camera ${(schedule as any).camera_id}`);
 ...
 */
app.post('/monitoring/initialize-enhanced', async (req: Request, res: Response) => {
  try {
    ...
    const baselineCameras = await AdaptiveMonitoringEngine.initializeNeighborhoodBaselines(mockNYCCameras);
    ...
    for (const baselineCamera of baselineCameras) {
      const schedule = {
        ...
        next_analysis_time: admin.firestore.Timestamp.fromDate(
          new Date(Date.now() + baselineCamera.baseline_score * 60 * 60 * 1000)
        ),
 ...
 */
app.get('/monitoring/status-enhanced', async (req: Request, res: Response) => {
  try {
    ...
    schedules.sort((a: any, b: any) => (a.current_score || 24) - (b.current_score || 24));
    
    // Group by sampling frequency ranges
    const frequencyGroups = {
      'critical_continuous': schedules.filter(s => (s as any).current_score <= 1),
      'hourly': schedules.filter(s => (s as any).current_score > 1 && (s as any).current_score <= 2),
      'bi_hourly': schedules.filter(s => (s as any).current_score > 2 && (s as any).current_score <= 4), 
      'four_hourly': schedules.filter(s => (s as any).current_score > 4 && (s as any).current_score <= 8),
      'eight_hourly': schedules.filter(s => (s as any).current_score > 8 && (s as any).current_score <= 16),
      'daily': schedules.filter(s => (s as any).current_score > 16 && (s as any).current_score <= 48),
      'low_frequency': schedules.filter(s => (s as any).current_score > 48)
    };
    ...
    return res.json({
      ...
      total_cameras: schedules.length,
      frequency_distribution: Object.fromEntries(
        Object.entries(frequencyGroups).map(([key, cameras]) => [key, cameras.length])
      ),
      most_frequent_cameras: schedules.slice(0, 5).map((s: any) => ({
      ...
      high_risk_zones: schedules.filter((s: any) => s.is_high_risk_zone).map((s: any) => ({
        camera_id: s.camera_id,
        name: s.camera?.name,
        neighborhood: s.neighborhood,
        current_score: s.current_score,
      ...
      neighborhoods_active: [...new Set(schedules.map((s: any) => s.neighborhood).filter(Boolean))],
      system_version: 'enhanced_adaptive_v2'
    });
...
async function processScheduledCameraAnalysisEnhanced(schedule: any): Promise<any> {
  console.log(`🔍 [ENHANCED] Processing adaptive analysis for camera ${schedule.camera?.name || schedule.camera_id as any as any}`);
  ...
  try {
    ...
    const cloudVisionAnalysis = await AdaptiveMonitoringEngine.eliminateFallbacks(async () => {
      return await CloudVisionService.analyzeCameraFeed(camera.imageUrl);
    });
    ...
    if (!cloudVisionAnalysis.success && cloudVisionAnalysis.error_type === 'analysis_failure') {
      console.error(`❌ [ENHANCED] All analysis attempts failed for ${schedule.camera_id as any as any}`);
    ...
    const neighborhoodBaseline = schedule.baseline_score as any || 24;
    const adaptiveScore = AdaptiveMonitoringEngine.calculateAdaptiveScore(
      cloudVisionAnalysis.numerical_data,
      camera,
      neighborhoodBaseline
    );
    
    console.log(`📊 [ENHANCED] Adaptive scoring results:`);
    console.log(`   🎯 Total Score: ${adaptiveScore.total_score.toFixed(2)}h (was ${schedule.current_score as any as any || 'unknown'}h)`);
    ...
    await storeViolationData(cloudVisionAnalysis.numerical_data, camera.name || schedule.camera_id as any as any);
    
    // Calculate next analysis time based on adaptive score
    const nextAnalysisTime = admin.firestore.Timestamp.fromDate(
      new Date(Date.now() + adaptiveScore.sampling_frequency_hours * 60 * 60 * 1000)
    );
    
    // Determine if significant score change occurred
    const previousScore = schedule.current_score as any as any || schedule.baseline_score as any;
    ...
    console.error(`❌ [ENHANCED] Enhanced analysis failed for ${schedule.camera_id as any as any}:`, error);
 ...
 */
async function initializeHighRiskStreetSegments(allCameras: any[]): Promise<any[]> {
  ...
  for (const zone of highRiskZones) {
    ...
    const matchingCameras = allCameras.filter(camera => 
      camera.area?.includes(zone.neighborhood) &&
      (camera.name?.includes('8th') || camera.name?.includes('9th') || camera.name?.includes('10th'))
    );
    ...
    if (matchingCameras.length > 0) {
      ...
      const schedule = {
        ...
        next_analysis_time: admin.firestore.Timestamp.fromDate(
          new Date(Date.now() + zone.auto_score * 60 * 60 * 1000)
        ),
        zone_classification: 'pedestrian_bike_intersection', 
 ...
 */
async function generateMockNYCCameraData(): Promise<any[]> {
  const mockCameras = [
 ...
 */
app.post('/monitoring/test-enhanced/:camera_id', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    ...
    const schedule = { id: scheduleDoc.docs[0].id, ...scheduleDoc.docs[0].data() };
...
app.get('/dashboard/camera-zones', async (req: Request, res: Response) => {
  try {
    ...
    const allSchedules = [
      ...originalSchedules.docs.map(doc => ({ id: doc.id, ...doc.data(), system: 'original' } as any)),
      ...enhancedSchedules.docs.map(doc => ({ id: doc.id, ...doc.data(), system: 'enhanced' } as any))
    ];
    ...
    for (const schedule of allSchedules) {
      const currentScore = schedule.current_score as any as any || schedule.baseline_score as any || 24;
      const samplingHours = schedule.sampling_frequency_hours || currentScore;
      
      // Get analysis statistics for this camera
      const analysisStats = await getCameraAnalysisStats(schedule.camera_id as any as any);
      
      const cameraZone = {
        camera_id: schedule.camera_id as any as any,
        camera_name: schedule.camera?.name || schedule.camera_id as any as any,
        neighborhood: schedule.neighborhood || schedule.camera?.area || 'Unknown',
        ...
        // Location data
        latitude: schedule.camera?.latitude || 40.7589,
        longitude: schedule.camera?.longitude || -73.9896,
    ...
    cameraZones.forEach(zone => {
      ...
      if ((frequencyBreakpoints as any)[tier]) {
        (frequencyBreakpoints as any)[tier].count++;
        (frequencyBreakpoints as any)[tier].cameras.push({
          camera_id: zone.camera_id,
 ...
 */
app.put('/dashboard/camera/:camera_id/frequency', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    ...
    const previousFrequency = currentSchedule?.current_score || currentSchedule?.sampling_frequency_hours || 24;
    
    // Calculate next analysis time
    const nextAnalysisTime = admin.firestore.Timestamp.fromDate(
      new Date(Date.now() + new_frequency_hours * 60 * 60 * 1000)
    );
    ...
    if (override_adaptive) {
      ...
      updateData.adaptive_override_until = admin.firestore.Timestamp.fromDate(
        new Date(Date.now() + 24 * 60 * 60 * 1000) // Override for 24 hours
      );
    }
 ...
 */
app.post('/dashboard/camera/:camera_id/reset-adaptive', async (req: Request, res: Response) => {
  try {
    const { camera_id } = req.params;
    ...
    const schedule = scheduleDoc.data();
    const baselineScore = schedule?.baseline_score || 24;
    ...
    await db.collection('monitoring_schedules').doc(camera_id).update({
      ...
      next_analysis_time: admin.firestore.Timestamp.fromDate(
        new Date(Date.now() + baselineScore * 60 * 60 * 1000)
      ),
 ...
 */
function categorizeFrequencyTier(hours: number): string {
  if (hours <= 1) return 'critical_continuous';
  if (hours <= 2) return 'hourly';
  if (hours <= 4) return 'bi_hourly';
  if (hours <= 8) return 'four_hourly';
  if (hours <= 16) return 'eight_hourly';
 ...
 */
function getFrequencyHeatmapColor(hours: number): string {
  // Red (critical) to Green (low frequency)
  if (hours <= 1) return '#ff0000';      // Critical red
  if (hours <= 2) return '#ff4500';      // Orange red
  if (hours <= 4) return '#ff8c00';      // Dark orange
  if (hours <= 8) return '#ffd700';      // Gold
  if (hours <= 16) return '#adff2f';     // Green yellow
  if (hours <= 48) return '#32cd32';     // Lime green
 ...
 */
async function getRecentDashboardActivity(): Promise<any[]> {
  try {
...
app.get('/dashboard/map-zones', async (req: Request, res: Response) => {
  try {
    ...
    for (const schedule of schedules) {
      ...
      const bounds = generateCameraZoneBounds(
        camera.latitude || 40.7589,
        camera.longitude || -73.9896,
        getZoneRadius(samplingHours) // Radius based on sampling frequency
      );
      
      const mapZone = {
        camera_id: schedule.camera_id as any as any,
        camera_name: camera.name || schedule.camera_id as any as any,
 ...
 */
function generateCameraZoneBounds(lat: number, lng: number, radiusKm: number): any {
 ...
 */
function getZoneRadius(samplingHours: number): number {
  // More frequent sampling = larger zone of influence
  if (samplingHours <= 1) return 0.8;     // Critical: 800m radius
  if (samplingHours <= 2) return 0.6;     // Hourly: 600m radius
  if (samplingHours <= 4) return 0.5;     // Bi-hourly: 500m radius
  if (samplingHours <= 8) return 0.4;     // Four-hourly: 400m radius
  if (samplingHours <= 16) return 0.3;    // Eight-hourly: 300m radius
 ...
 */
function calculateMapCenter(zones: any[]): { lat: number; lng: number } {
  if (zones.length === 0) {
 ...
 */
function calculateMapBounds(zones: any[]): any {
  if (zones.length === 0) {
    return {
      north: 40.8,
      south: 40.7,
 ...
 */
async function getCameraAnalysisStats(cameraId: string): Promise<any> {
  try {
    const now = new Date();
    ...
    const allAnalyses = allAnalysesSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    } as any));
    
    // Get today's analyses
    const todayAnalyses = allAnalyses.filter((analysis: any) => {
      const analysisDate = analysis.timestamp?.toDate ? analysis.timestamp.toDate() : new Date(analysis.timestamp);
      return analysisDate >= todayStart;
    });
    
    // Get last analysis
    const lastAnalysis = allAnalyses.length > 0 ? allAnalyses[0] : null;
    const lastAnalysisTime = lastAnalysis?.timestamp?.toDate ? 
      (lastAnalysis.timestamp as any).toDate() : 
      (lastAnalysis?.timestamp ? new Date(lastAnalysis.timestamp) : null);
 ...
 */
function calculateAverageAnalysesPerDay(analyses: any[]): number {
  if (analyses.length === 0) return 0;
  
  const firstAnalysis = analyses[analyses.length - 1];
  const lastAnalysis = analyses[0];
  
  const firstDate = firstAnalysis.timestamp?.toDate ? 
    firstAnalysis.timestamp.toDate() : new Date(firstAnalysis.timestamp);
  const lastDate = lastAnalysis.timestamp?.toDate ? 
    (lastAnalysis.timestamp as any).toDate() : new Date(lastAnalysis.timestamp);
 ...
 */
app.post('/generate-camera-network', async (req: Request, res: Response) => {
  try {
    ...
    const network = generateManhattanCameraNetwork(camera_count);
    
    // Store network in Firestore
    await db.collection('camera_networks').doc('manhattan_primary').set({
      ...network,
      created_at: admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
app.get('/camera-network', async (req: Request, res: Response) => {
  try {
    ...
    result.data.network_metadata = networkData?.network_metadata;
 ...
 */
app.get('/voronoi-map', async (req: Request, res: Response) => {
  try {
    const { 
      zoom_level = 12, 
      bbox,
      include_analysis_stats = true,
    ...
    for (const doc of territoriesSnapshot.docs) {
      ...
      const mapTerritory = {
        ...
        properties: {
          ...
          risk_score: territory.risk_analysis.current_risk_score,
          violation_density: territory.risk_analysis.violation_density
        },
...
app.get('/territory/:territoryId', async (req: Request, res: Response) => {
  try {
    ...
    // Get camera metadata
    const cameraDoc = await db.collection('camera_metadata').doc((territory as any).camera_id).get();
    const camera = cameraDoc.exists ? cameraDoc.data() : null;
    
    // Get analysis statistics
    const analysisStats = await getCameraAnalysisStats((territory as any).camera_id);
    
    // Get recent violations in this territory
    const violationsSnapshot = await db.collection('violation_events')
      .where('camera_id', '==', (territory as any).camera_id)
      .orderBy('timestamp', 'desc')
      .limit(10)
      .get();
    
    const recentViolations = violationsSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
    
    // Get neighboring territories analysis
    const neighboringAnalysis = await Promise.all(
      (territory as any).neighbors.map(async (neighbor: any) => {
...
function simplifyGeometry(geometry: any, zoomLevel: number): any {
  // Simplify polygon geometry based on zoom level
  // At lower zoom levels, reduce vertex count for better performance
  if (zoomLevel < 10) {
    const coords = geometry.coordinates[0];
    const simplified = coords.filter((_: any, index: number) => index % 2 === 0);
    return {
      ...geometry,
...
function calculateCoverageEfficiency(territory: any, analysisStats: any): number {
...
function calculateRiskTrend(violations: any[]): 'increasing' | 'decreasing' | 'stable' {
...
function calculateOptimalFrequency(territory: any, analysisStats: any): number {
...
/**
 * CAMERA RESTORATION - Import all 907 cameras from zone-lookup.json
 */
app.post('/restore-cameras', async (req: Request, res: Response) => {
  try {
    console.log('🚀 Starting bulk camera restoration from zone-lookup.json...');
    
    // Load zone lookup data with all 907 cameras
    const fs = require('fs');
    const path = require('path');
    
    // In Cloud Functions, files are in the deployed directory
    const zoneLookupPath = path.join(__dirname, '../../../data/zone-lookup.json');
    
    let zoneLookup: any;
    try {
      const zoneLookupData = fs.readFileSync(zoneLookupPath, 'utf8');
      zoneLookup = JSON.parse(zoneLookupData);
    } catch (error) {
      console.error('❌ Could not load zone-lookup.json:', error);
      return res.status(500).json({
        error: 'Zone lookup data not found',
        details: 'zone-lookup.json file is missing or corrupted'
      });
    }
    
    console.log(`📂 Loaded ${Object.keys(zoneLookup).length} cameras from zone-lookup.json`);
    
    // Create monitoring schedules for ALL cameras
    ...
    for (let i = 0; i < allSchedules.length; i += batchSize) {
      try {
        ...
        currentBatch.forEach(schedule => {
          ...
          batch.set(docRef, schedule, { merge: true }); // Use merge to avoid overwriting existing data
        });
    ...
    return res.json({
      ...
      next_steps: [
        ...
        'Test endpoint functionality with restored cameras'
      ]
    });
