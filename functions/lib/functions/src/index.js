...
app.get('/monitoring/camera-image/:cameraId', async (req, res) => {
    ...
    try {
        ...
        const zoneInfo = await (0, camera_processing_1.getCameraZoneInfo)(cameraData);
        ...
        try {
            nycUuid = await (0, camera_processing_1.getCameraUuid)(cameraData, exports.db);
        ...
        if (!processingError && visionResults.numerical_data) {
            try {
                ...
                const adaptiveScore = AdaptiveMonitoringEngine.calculateAdaptiveScore(visionResults.numerical_data, cameraData, 24);
...
// ANALYTICS ENDPOINTS 
// =====================================================
app.get('/analytics/stored-analyses', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 20;
        // Get recent analyses from Firestore
        const analysesSnapshot = await exports.db.collection('analyses')
            .orderBy('timestamp', 'desc')
            .limit(limit)
            .get();
        const analyses = analysesSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        // Get processing statistics
        const totalCount = await exports.db.collection('analyses').count().get();
        const successfulAnalyses = analyses.filter((a) => !a.processing_error);
        const stats = {
            ...
            avg_processing_time: analyses.length > 0 ? Math.round(analyses.reduce((sum, a) => sum + (a.processing_time_ms || 0), 0) / analyses.length) : 0,
            avg_temperature_score: analyses.length > 0 ? (analyses.reduce((sum, a) => sum + (a.temperature_score || 0), 0) / analyses.length).toFixed(1) : '0'
        };
        return res.json({
            success: true,
            analyses,
...
app.get('/analytics/processing-queue', async (req, res) => {
    try {
        // Get cameras that need processing (no recent analysis)
        const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
        const camerasSnapshot = await exports.db.collection('monitoring_schedules')
            .where('last_analysis_time', '<', oneHourAgo)
            .limit(10)
            .get();
        const queuedCameras = camerasSnapshot.docs.map(doc => (Object.assign(Object.assign({ camera_id: doc.id }, doc.data()), { queue_status: 'pending' })));
        return res.json({
            success: true,
 ...
 */
app.get('/dashboard/nyc-cameras', async (req, res) => {
    try {
        ...
        const cameras = Object.values(zoneLookupData).map((zone) => ({
        ...
        const mapCenter = {
            lat: cameras.reduce((sum, camera) => sum + camera.latitude, 0) / cameras.length,
            lng: cameras.reduce((sum, camera) => sum + camera.longitude, 0) / cameras.length
        };
 ...
 */
app.delete('/cleanup/fake-cameras', async (req, res) => {
    ...
    try {
        ...
        for (const cameraId of fakeCameras) {
            const docRef = exports.db.collection('monitoring_schedules').doc(cameraId);
            ...
            if (doc.exists) {
                ...
                console.log(`✅ [FOUND] ${cameraId} - ${((_b = (_a = doc.data()) === null || _a === void 0 ? void 0 : _a.camera) === null || _b === void 0 ? void 0 : _b.name) || 'No name'}`);
            }
            else {
        ...
        for (const camera of existingCameras) {
            try {
                await exports.db.collection('monitoring_schedules').doc(camera.id).delete();
                deletionResults.push({
                    camera_id: camera.id,
                    collection: 'monitoring_schedules',
                    status: 'deleted',
                    camera_name: ((_d = (_c = camera.data) === null || _c === void 0 ? void 0 : _c.camera) === null || _d === void 0 ? void 0 : _d.name) || 'Unknown'
                });
                console.log(`🗑️ [DELETED] monitoring_schedules/${camera.id} - ${((_f = (_e = camera.data) === null || _e === void 0 ? void 0 : _e.camera) === null || _f === void 0 ? void 0 : _f.name) || 'Unknown'}`);
            }
            catch (error) {
                deletionResults.push({
                    camera_id: camera.id,
        ...
        for (const camera of existingCameras) {
            try {
                ...
                const analysesQuery = await exports.db.collection('analyses')
 ...
 */
app.post('/restore/complete-cameras', async (req, res) => {
    try {
        ...
        const existingSnapshot = await exports.db.collection('monitoring_schedules').get();
        ...
        for (const borough of boroughs) {
            ...
            for (let i = 1; i <= 181; i++) { // 181 * 5 = 905 cameras total
                ...
                cameraIndex++;
                if (enhancedCameras.length >= 900)
                    break; // Cap at 900 cameras
            }
            if (enhancedCameras.length >= 900)
                break;
        }
        ...
        if (existingCount > 0) {
            const deleteBatch = exports.db.batch();
        ...
        for (let i = 0; i < enhancedCameras.length; i += batchSize) {
            ...
            currentBatch.forEach(zone => {
                const docRef = exports.db.collection('monitoring_schedules').doc();
        ...
        for (let i = 0; i < enhancedCameras.length; i += batchSize) {
            ...
            currentBatch.forEach(zone => {
                const docRef = exports.db.collection('voronoi_territories').doc(zone.zone_id);
                const territory = {
                    zone_id: zone.zone_id,
                    camera_id: zone.camera_id,
                    camera_name: zone.camera_name,
        ...
        const verifySnapshot = await exports.db.collection('monitoring_schedules').get();
