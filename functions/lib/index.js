...
app.get('/metrics/recent-analysis', async (req, res) => {
    try {
        ...
        res.json({ recent_analyses: analyses, count: analyses.length });
    }
    catch (error) {
        res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
    }
...
async function identifyCriticalCameraZones() {
    ...
    const sampleCriticalZones = [
        ...
        },
        {
            ...
            camera: {
                id: 'cam_amsterdam_001',
                name: 'Amsterdam Ave - 96th St',
                latitude: 40.7831,
                longitude: -73.9441,
...
app.get('/dashboard/camera-zones', async (req, res) => {
    try {
        ...
        const cameraZones = allSchedules.map((schedule) => {
            var _a, _b, _c, _d;
...
app.get('/dashboard/camera/:camera_id/image', async (req, res) => {
    ...
    try {
        ...
        if (!imageUrl) {
            res.status(404).json({ error: 'Camera image URL not available' });
            return;
        }
        ...
        if (!imageResponse.ok) {
            res.status(imageResponse.status).json({ error: 'Failed to fetch camera image' });
            return;
        }
        // Forward the image
